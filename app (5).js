...
function openAddSheetModal(mfrId) {
  ...
  `;
  const modal = document.getElementById('sheet-modal-body');
  if (modal) {
    modal.innerHTML = html;
    document.getElementById('modal-sheet').classList.add('show');
    
    const cancelBtn = document.getElementById('cancel-sheet');
    if (cancelBtn) {
...
function openEditSheetModal(sheetId) {
  ...
  const html = `
    ...
    <button class="btn-danger" style="width:100%;margin-top:8px;text-align:center;color:#F87171" onclick="if(confirm('Delete this sheet?')){state.sheets=state.sheets.filter(s=>s.id!=='${esc(sheetId)}');save();const m=document.getElementById('modal-sheet');if(m)m.classList.remove('show');render();toast('Sheet deleted')}">Delete Sheet</button>
  `;
  const modal = document.getElementById('sheet-modal-body');
  if (modal) {
    modal.innerHTML = html;
...
function saveSheet() {
  ...
  if (!nameEl || !dateEl || !itemsEl) return;
  ...
  if (state.editSheetId) {
    ...
    // Add new
    const mfrId = document.querySelector('[data-add-sheet]')?.dataset.addSheet;
...
function render() {
  ...
  if (!content || !title) return; // Safety check
